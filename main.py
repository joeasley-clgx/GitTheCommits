...
def generate_excel_file(commit_list: list) -> None:
    ...
    with xlsxwriter.Workbook(f"output.xlsx") as workbook:
        ...
        if commit_detail_visibilty["Message"]: 
            letter = letter_dictionary[column_number]
        ...
        for commit in commit_list:
            column_number = 0
            if commit_detail_visibilty["Message"]: 
                worksheet.write_string(row, column_number, commit['message'], data_format)
                column_number += 1
            if commit_detail_visibilty["ItemNumber"]:
                worksheet.write_string(row, column_number, commit['item number'], centered_data_format) 
                column_number += 1
            if commit_detail_visibilty["Author"]:
                worksheet.write_string(row, column_number, commit['author'], data_format) 
                column_number += 1
            if commit_detail_visibilty["Date"]:
                worksheet.write_string(row, column_number, str(commit['date']), centered_data_format) 
                column_number += 1
            if commit_detail_visibilty["CommitUrl"]:
                worksheet.write_string(row, column_number, commit['commit url'], data_format) 
                column_number += 1
            if commit_detail_visibilty["PullRequestUrl"]:
                worksheet.write_string(row, column_number, commit['pr url'], data_format) 
                column_number += 1
            if commit_detail_visibilty["Sha"]:
                worksheet.write_string(row, column_number, commit['sha'], centered_data_format) 
                column_number += 1
